...
contract BondDepositoryRai is Ownable {
  ...
  address public treasury; // mints RAI when receives principle
  ...
  struct Bond {
    ...
    uint pricePaid; // In RAI, for front end viewing
  }
   ...
   */
   function setContract(address _address,CONTRACTID _contract ) external onlyPolicy() {
        require(_address != address(0));
        if( _contract == CONTRACTID.INVITEID ) { // 0
            community = _address;
        } else if ( _contract == CONTRACTID.REWARDDISTRIBUTORID ) { // 1
            ...
            treasury = _address;
        } else if ( _contract == CONTRACTID.PRINCIPLE ) { // 6
            principle = _address;
        } else if ( _contract == CONTRACTID.CALCULATOR ) { // 6
  ...
  /**
   *  @notice converts bond price to RAI value
   *  @return price_ uint
   */
